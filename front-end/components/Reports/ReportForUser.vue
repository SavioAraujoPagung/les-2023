        ...
        <div style="display: none;z-index:2000;" id="modalReportForCustomer" class="bg-white position-fixed w-100 h-100 start-0 top-0 align-items-center justify-content-center">
            <div class="w-100 p-5">
                ...
                <div class="mb-3 p-3">
                    ...
                    <div v-for="(information, i) in dataUser" :key="i" :id="'information'+i" class="mb-3">
                        <h5 class="fw-bold text-uppercase text-center w-100 bg-light p-3 border border-1 border-default">{{ information.customer }} - {{ formatDate(new Date(information.date)) }} - {{information.totalValue}}</h5>
                    </div>
...
import { useReportStore } from "~~/stores/ReportStore";
    ...
    const { dataUser } = storeToRefs(useReportStore());
    const { getReportForUsers } = useReportStore();

    const reportStart = ref('');
    const reportEnd = ref('');

    const formSave = async () => {
        getReportForUsers(reportStart.value, reportEnd.value);
